...
import com.xiuman.xingduoduo.util.Common;

/**
 * 自定义ImageView ，可对图片进行多点触控缩放和拖动
 * 
 * @author danding
 * 
 */
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			if (event.getPointerCount() == 1) {
				...
				}else if(totalTranslateX - movedDistanceX>-5&&totalTranslateX - movedDistanceX<0){
					getParent().requestDisallowInterceptTouchEvent(false);
				}
		...
		if (sourceBitmap != null) {
			...
			if (bitmapWidth > width || bitmapHeight > height) {
				...
				if (bitmapWidth - width > bitmapHeight - height) {
					...
					// 当图片高度大于屏幕高度时，将图片等比例压缩，使它可以完全显示出来
					float ratio = (height-70) / (bitmapHeight * 1.0f);
					matrix.postScale(ratio, ratio);
					float translateX = ((width-40) - (bitmapWidth * ratio)) / 2f;
					// 在横坐标方向上进行偏移，以保证图片居中显示
					matrix.postTranslate(translateX, 0);
					totalTranslateX = translateX;
					totalRatio = initRatio = ratio;
